...
%   Whether to return vectors aligned to spherical coordinate axes (true) or cartesian (false).
    ...
    if strcmp(parentName, 'Saturn')
        ...
        if strcmp(parentName, 'Uranus')
            % Some Uranus MP models use the Ness et al. (1986) offset, 
            % tilted dipole model (OTD) location for the USM origin.
            % See https://doi.org/10.1126/science.233.4759.85 .
    ...
    if ~strcmp(MPmodel, 'None')
        ...
        if strcmp(MPmodel, 'AB2005') || strcmp(MPmodel, 'Bode1994') || contains(MPmodel, 'Engle1992')
        ...
        elseif strcmp(MPmodel, 'Q3mp') || strcmp(MPmodel, 'SM1996')
            ...
            elseif strcmp(parentName, 'Neptune')
                ...
                Rss_Rp = 25; % Fixed value suggested by Masters (2014) based on synthesizing Voyager results
            ...
            % Combine according to the rule described in SM1996 and scale
            % to dipole moment. Negate the expression because SM1996 was
            % Earth-focused, in that they defined the z axis to be opposite
            % the dipole moment.
        ...
        switch(parentName)
            ...
            case 'Saturn'
                ...
                % Paraboloid model of Arridge et al. (2006):
                % https://doi.org/10.1029/2005JA011574
                coeffs = dlmread(fullfile([coeffPath 'coeffsSaturnMpauseAea2006.csv']), ',', nHeadLines, 0);
                a = coeffs(1,1:4);
...
function [Br, Bth, Bphi] = MPsphericalHarmonic(r_Rss, thMP, phiMP, Gnm, Nmax)
    ...
    for n=1:min([10, Nmax])
        ...
        for m=1:n
            ...
            dBphi = -m*(r_Rss).^(n-1) .* squeeze(Gnm(n,m+1,:))' .*  Pnm ./ sin(thMPadj) .* sin(m*phiMP);
...
function [Bx, By, Bz] = MPboxHarmonic(xyzPSM_Rp, Psi, a, b, c, d, pqrs)
    % Tsyganenko (2002) model (https://doi.org/10.1029/2001JA000219) as
    % formulated by Arridge and Eggington (2022)
    ...
    for i=1:3
        for k=1:3
            ...
            dBperpx = -prScale * abRot .* exp(xyzPSM_Rp(1,:)*prScale) .* cos(xyzPSM_Rp(2,:)/p(i)) .* sin(xyzPSM_Rp(3,:)/r(i));
            dBperpy =   1/p(i) * abRot .* exp(xyzPSM_Rp(1,:)*prScale) .* sin(xyzPSM_Rp(2,:)/p(i)) .* sin(xyzPSM_Rp(3,:)/r(i));
            dBperpz =  -1/r(k) * abRot .* exp(xyzPSM_Rp(1,:)*prScale) .* cos(xyzPSM_Rp(2,:)/p(i)) .* cos(xyzPSM_Rp(3,:)/r(i));
    ...
    for j=1:4
        for l=1:4
            ...
            dBparax = -qsScale * cdRot .* exp(xyzPSM_Rp(1,:)*qsScale) .* cos(xyzPSM_Rp(2,:)/q(j)) .* cos(xyzPSM_Rp(3,:)/s(l));
            dBparay =   1/q(j) * cdRot .* exp(xyzPSM_Rp(1,:)*qsScale) .* sin(xyzPSM_Rp(2,:)/q(j)) .* cos(xyzPSM_Rp(3,:)/s(l));
            dBparaz =   1/s(l) * cdRot .* exp(xyzPSM_Rp(1,:)*qsScale) .* cos(xyzPSM_Rp(2,:)/q(j)) .* sin(xyzPSM_Rp(3,:)/s(l));
