...
for i=2:-1:1
    ...
    [rMinMoon_km, rP_km{i}] = GetMoonDist(sc, parentName, ets{i});
    ...
    for opt=opts
        for MPopt=MPopts(2:end)
            GetBplotAndLsq(ets{i}, t_h{i}, r_km, theta, phi, xyz_km, BrSC{i}, BthSC{i}, BphiSC{i}, ...
                scName(i), parentName, spkParent, orbStr, opt, MPopt, SEQUENTIAL);
        end
    end
...
legend([scTraj{1} scTraj{2}], scName)
